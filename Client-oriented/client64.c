...
#endif
        fd0 = open("ionode64_0",O_RDWR|O_CREAT|O_TRUNC, 0644);
        fd1 = open("ionode64_1",O_RDWR|O_CREAT|O_TRUNC, 0644);
...
int child(int n, pid_t cpid[4], int p[2]){
        ...
        int readbuf[(LEN/4)*SIZE], rpbuf[(LEN/4)*SIZE], rfc[LEN*SIZE], ctoi[2], fd0, fd1;
        char name[BUFSIZ];
        int i, j, k, m, cnt[3], flag, semid;
